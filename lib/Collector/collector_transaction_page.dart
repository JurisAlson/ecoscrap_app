// IMPORTANT FIXES:
// 1. sellHistoryStream is now inside build(), after collectorId exists.
// 2. _SellFormState now has its own _formatDate().
// 3. _SellFormState now has its own _infoRow().
// 4. Mores Scrap Logs are shown below SEND TO MORES SCRAP.
...
class _CollectorTransactionPageState extends State<CollectorTransactionPage> {
  ...
  void initState() {
    super.initState();
    _tab = (widget.requestId != null && widget.requestId!.trim().isNotEmpty)
        ? 0
        : 1;
  }
...
Future<void> _saveBuyReceipt(Map<String, dynamic> requestData) async {
  ...
  Widget build(BuildContext context) {
    ...
    if (collectorId == null) {
      return _empty(title: "Not signed in", body: "Please sign in to continue.");
    }
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.shopping_bag_rounded, color: primaryColor),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
  ...
  Widget _historySection(AsyncSnapshot<QuerySnapshot> historySnap) {
    ...
    final totalPages =
        totalItems == 0 ? 1 : (totalItems / _historyItemsPerPage).ceil();
        ...
        if (historySnap.connectionState == ConnectionState.waiting)
          ...
          ...pageDocs.map((doc) {
            ...
            return Padding(
              ...
              child: _historyCard(
                householdName: (item['householdName'] ?? 'Household').toString(),
                bagLabel: (item['bagLabel'] ?? 'Bag').toString(),
                kg: ((item['kg'] as num?) ?? 0).toDouble(),
                pricePerKg: ((item['pricePerKg'] as num?) ?? 0).toDouble(),
                totalAmount: ((item['totalAmount'] as num?) ?? 0).toDouble(),
    ...
    return Row(
      ...
      children: [
        _pageArrowButton(
          ...
          onTap: () {
            if (_historyPage > 0) setState(() => _historyPage--);
          },
        ),
        const SizedBox(width: 8),
        ...pages.map((pageIndex) {
          return Padding(
            ...
            child: _pageNumberButton(
              label: "${pageIndex + 1}",
              selected: pageIndex == _historyPage,
        ...
        _pageArrowButton(
          ...
          onTap: () {
            if (_historyPage < totalPages - 1) setState(() => _historyPage++);
          },
        ),
      ],
    );
  ...
  List<int> _visiblePages(int totalPages, int currentPage) {
    if (totalPages <= 3) return List.generate(totalPages, (i) => i);
    if (currentPage <= 1) return [0, 1, 2];
  ...
  Widget _pageArrowButton({
    ...
    return InkWell(
      ...
      child: Container(
        ...
        alignment: Alignment.center,
        decoration: BoxDecoration(
          color: enabled ? Colors.white.withOpacity(0.06) : Colors.white.withOpacity(0.03),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: Colors.white.withOpacity(0.08)),
        ),
...
class _SellFormState extends State<_SellForm> {
  ...
  static const int _sellHistoryItemsPerPage = 5;
  int _sellHistoryPage = 0;
  ...
  Widget _buildSellHistoryPagination(int totalPages) {
  if (totalPages <= 1) return const SizedBox.shrink();

  final pages = _visibleSellPages(totalPages, _sellHistoryPage);

  return Row(
    mainAxisAlignment: MainAxisAlignment.center,
    children: [
      _sellPageArrowButton(
        icon: Icons.chevron_left_rounded,
        enabled: _sellHistoryPage > 0,
        onTap: () {
          if (_sellHistoryPage > 0) {
            setState(() => _sellHistoryPage--);
          }
        },
      ),
      const SizedBox(width: 8),
      ...
      ...pages.map((pageIndex) {
        final isSelected = pageIndex == _sellHistoryPage;

        return Padding(
          padding: const EdgeInsets.symmetric(horizontal: 4),
          child: _sellPageNumberButton(
            label: "${pageIndex + 1}",
            selected: isSelected,
            onTap: () => setState(() => _sellHistoryPage = pageIndex),
          ),
        );
      }),
      ...
      _sellPageArrowButton(
        icon: Icons.chevron_right_rounded,
        enabled: _sellHistoryPage < totalPages - 1,
        onTap: () {
          if (_sellHistoryPage < totalPages - 1) {
            setState(() => _sellHistoryPage++);
          }
        },
      ),
...
List<int> _visibleSellPages(int totalPages, int currentPage) {
  if (totalPages <= 3) {
...
Widget _sellPageArrowButton({
  ...
  Future<void> _confirmAndSubmitSell(double currentKg) async {
    ...
    if (ok == true) await _submitSell(currentKg);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final sellHistoryStream = FirebaseFirestore.instance
        .collection('Users')
        .doc(collectorId)
        .collection('transactions')
        .where('kind', isEqualTo: 'sell_request')
        .orderBy('createdAt', descending: true)
        .limit(50)
        .snapshots();
      ...
      builder: (context, inventorySnap) {
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, activeSnap) {
            ...
            final activeDoc =
                hasActiveMoresTxn ? activeSnap.data!.docs.first : null;
              ...
              child: Column(
                ...
                children: [
                          ...
                          : () async {
                              await Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (_) => CollectorTrackingPage(
                                    fixedDestination:
                                        const LatLng(moresLat, moresLng),
                                    destinationTitle: "Mores Scrap",
                                    destinationAddress: "Mores Scrap",
                                    trackingType: "sell",
                                    showChatButton: true,
                  ...
                  StreamBuilder<QuerySnapshot>(
                    stream: sellHistoryStream,
                    builder: (context, sellHistorySnap) {
                      return _sellHistorySection(sellHistorySnap);
                    },
                  ),
                ],
              ),
...
Widget _sellHistorySection(
  AsyncSnapshot<QuerySnapshot> historySnap,
) {
  final historyDocs = historySnap.data?.docs ?? [];
  ...
  final totalPages = totalItems == 0
      ? 1
      : (totalItems / _sellHistoryItemsPerPage).ceil();

  if (_sellHistoryPage >= totalPages) {
    _sellHistoryPage = totalPages - 1;
  }
  ...
  final startIndex = _sellHistoryPage * _sellHistoryItemsPerPage;
  final endIndex = (startIndex + _sellHistoryItemsPerPage) > totalItems
      ? totalItems
      : (startIndex + _sellHistoryItemsPerPage);
  ...
  return GestureDetector(
    onHorizontalDragEnd: (details) {
      ...
      if (velocity < 0 && _sellHistoryPage < totalPages - 1) {
        setState(() => _sellHistoryPage++);
      } else if (velocity > 0 && _sellHistoryPage > 0) {
        setState(() => _sellHistoryPage--);
      }
    },
    child: Column(
      ...
      children: [
        ...
        const Text(
          "Latest transactions sent to Mores Scrap.",
          style: TextStyle(
        ...
        if (historySnap.connectionState == ConnectionState.waiting)
          ...
          _TransactionUi.glassCard(
            child: const Padding(
              ...
              child: Center(
                child: Text(
                  "No Mores Scrap transactions yet.",
                  style: TextStyle(color: Colors.white70),
                ),
              ),
            ),
          ...
          ...pageDocs.map((doc) {
            ...
            final status = (item['status'] ?? 'incoming').toString();

            final createdAt = item['createdAt'] as Timestamp?;

            final type = (item['bagLabel'] ??
                    item['plasticType'] ??
                    item['materialType'] ??
                    item['type'] ??
                    'Plastic')
            ...
            final pricePerKg =
                ((item['pricePerKg'] as num?) ??
                        (item['moresPricePerKg'] as num?) ??
                        0)
                    .toDouble();

            final totalAmount =
                ((item['totalAmount'] as num?) ??
                        (item['amount'] as num?) ??
                        (item['moresTotalAmount'] as num?) ??
                        0)
                    .toDouble();

            return Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: _sellHistoryCard(
                kg: kg,
                status: status,
                createdAt: createdAt,
                type: type,
          ...
          _buildSellHistoryPagination(totalPages),
        ],
      ],
    ),
  );
