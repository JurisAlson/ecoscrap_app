...
class _DashboardPageState extends State<DashboardPage> {
      ...
      child: ListView(
        ...
        children: [
          ...
          _plasticTypeCard(
            ...
            name: "White Plastics",
            note: "Solid white plastic items commonly used for containers.",
            exampleLabels: const [
              "Example 1",
              "Example 2",
              "Example 3",
          ...
          _plasticTypeCard(
            ...
            note: "Clear plastic that you can see through, such as drink bottles.",
            exampleLabels: const [
              "Example 1",
              "Example 2",
              "Example 3",
        ...
        children: [
          ...
          GridView.count(
            ...
            children: [
              ...
              _exampleSlot(
                imagePath: exampleImages.length > 1 ? exampleImages[1] : null,
              ),
              _exampleSlot(
                imagePath: exampleImages.length > 2 ? exampleImages[2] : null,
              ),
              _exampleSlot(
                imagePath: exampleImages.length > 3 ? exampleImages[3] : null,
              ),
            ],
          ),
          const SizedBox(height: 10),
      ...
      child: Container(
        ...
        child: ClipRRect(
          ...
          child: imagePath != null
              ...
              : const Center(
                  child:
                      Icon(Icons.photo_outlined, color: Colors.white38, size: 22),
                ),
        ),
      ),
    );
  ...
  Widget _historyScreen() {
    ...
    final pickupQuery = FirebaseFirestore.instance
        .collection('requests')
        .where('type', isEqualTo: 'pickup')
        .where('householdId', isEqualTo: user.uid)
    ...
    final dropoffQuery = FirebaseFirestore.instance
        .collection('dropoff_requests')
        .where('householdId', isEqualTo: user.uid)
        .where('active', isEqualTo: false)
        .orderBy('updatedAt', descending: true)
          ...
          Expanded(
            child: StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, userSnap) {
                ...
                return StreamBuilder<QuerySnapshot>(
                  stream: pickupQuery.snapshots(),
                  builder: (context, pickupSnap) {
                    return StreamBuilder<QuerySnapshot>(
                      stream: dropoffQuery.snapshots(),
                      builder: (context, dropoffSnap) {
                        if (pickupSnap.connectionState ==
                                ConnectionState.waiting ||
                            dropoffSnap.connectionState ==
                                ConnectionState.waiting) {
                          return const Center(
                            child: CircularProgressIndicator(),
                          );
                        ...
                        if (pickupSnap.hasError) {
                          return Center(
                            child: Text(
                              "Error loading pickup history: ${pickupSnap.error}",
                        ...
                        if (dropoffSnap.hasError) {
                          return Center(
                            child: Text(
                              "Error loading drop-off history: ${dropoffSnap.error}",
                              style: const TextStyle(color: Colors.white70),
                              textAlign: TextAlign.center,
                            ),
                          );
                        ...
                        final entries = <_HistoryEntry>[
                          ..._buildPickupHistoryEntries(
                            pickupSnap.data?.docs ?? [],
                            lastHistoryCleared,
                          ),
                          ..._buildDropoffHistoryEntries(
                            dropoffSnap.data?.docs ?? [],
                            lastHistoryCleared,
                          ),
                        ];
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (_, i) => entries[i].card,
                        );
                      },
                    );
                  },
  ...
  List<_HistoryEntry> _buildPickupHistoryEntries(
    List<QueryDocumentSnapshot> docs,
    Timestamp? lastHistoryCleared,
  ) {
    final entries = <_HistoryEntry>[];
    ...
    for (final doc in docs) {
      ...
      if (!_shouldIncludeHistory(updatedAt, lastHistoryCleared)) continue;

      entries.add(
        _HistoryEntry(
          timestamp: updatedAt!,
          card: _pickupHistoryCard(doc),
        ),
      );
    }
  ...
  List<_HistoryEntry> _buildDropoffHistoryEntries(
    List<QueryDocumentSnapshot> docs,
    Timestamp? lastHistoryCleared,
  ) {
    final entries = <_HistoryEntry>[];
  ...
  Widget _pickupHistoryCard(QueryDocumentSnapshot d) {
    ...
    return _historyCard(
      title: _pickupHistoryTitle(status),
      subtitle: subtitleLines.join("\n"),
      rightText: _formatShortTime(ts),
      icon: _pickupHistoryIcon(status),
      iconBg: _pickupHistoryIconBg(status),
      iconColor: _pickupHistoryIconColor(status),
    );
  }

  Widget _dropoffHistoryCard(QueryDocumentSnapshot d) {
    final data = d.data() as Map<String, dynamic>;

    final junkshopName = (data['junkshopName'] ?? '—').toString();
    final status = (data['status'] ?? '').toString().toLowerCase();
    ...
    final subtitleLines = [
      "Type: Drop-off",
      "Destination: $junkshopName",
      "Created: ${_formatDateTime(ts)}",
      "Status: ${status.isEmpty ? '—' : status}",
    ];

    return _historyCard(
      title: _dropoffHistoryTitle(status),
      subtitle: subtitleLines.join("\n"),
      rightText: _formatShortTime(ts),
      icon: _dropoffHistoryIcon(status),
      iconBg: _dropoffHistoryIconBg(status),
      iconColor: _dropoffHistoryIconColor(status),
    );
  }

  String _pickupHistoryTitle(String status) {
    switch (status) {
      case 'accepted':
  ...
  IconData _pickupHistoryIcon(String status) {
    switch (status) {
      case 'completed':
  ...
  Color _pickupHistoryIconBg(String status) {
    switch (status) {
      case 'completed':
  ...
  Color _pickupHistoryIconColor(String status) {
    switch (status) {
      case 'completed':
        return Colors.green;
      case 'accepted':
  ...
  String _dropoffHistoryTitle(String status) {
    switch (status) {
      case 'pending':
        return "Drop-off request sent";
      case 'en_route':
        return "Drop-off in progress";
      case 'completed':
      ...
      default:
        return status.isEmpty ? "Drop-off update" : "Drop-off $status";
    }
  }

  IconData _dropoffHistoryIcon(String status) {
    switch (status) {
      case 'completed':
        return Icons.check_circle_outline;
      case 'en_route':
  ...
  Color _dropoffHistoryIconBg(String status) {
    switch (status) {
      case 'completed':
        return Colors.green.withOpacity(0.15);
      case 'en_route':
  ...
  Color _dropoffHistoryIconColor(String status) {
    switch (status) {
      case 'completed':
        return Colors.green;
      case 'en_route':
          ...
          Expanded(
            child: StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, userSnap) {
                final userData =
                    userSnap.data?.data() as Map<String, dynamic>? ?? {};
                final lastCleared = userData['lastNotifClearedAt'] as Timestamp?;
    ...
    return Container(
      ...
      child: const Column(
        ...
        children: [
          ...
          _HowToRow(
            icon: Icons.history,
            text: "Compare plastics on the Home page before deciding to scan.",
          ),
          SizedBox(height: 10),
          _HowToRow(
